...
u32 ps2_init()
{
	...
	rvid_printf("  Writing null CCB...\n");
	ps2_write_ccb(ps2_null_ccb());

	/* Perform self-check */
	rvid_printf("  Self-check...\n");
	...
	if (!has_port1 && !has_port2) {
		rvid_printf("  No ports available!\n");
		return E_NO;
	}
